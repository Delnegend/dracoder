...
mod params {
    ...
    pub fn x264_ll<'a>(in_path: &'a str, out_path: &'a str) -> Vec<&'a str> {
        vec!["-i", in_path, "-c:v", "libx264", "-vf", "scale=ceil(iw/2)*2:ceil(ih/2)*2", "-pix_fmt", "yuv420p", "-tag:v", "hvc1", "-crf", "0", "-c:a", "copy", out_path]
    }
...
fn get_args(in_path: &str) -> Vec<String> {
    ...
    match format {
        ...
        Formats::X264LL => params::x264_ll(in_path, &out_path),
